...
import { History, Share2, AlertCircle, Wifi, Clock, CheckCircle2, X, Ticket, Send, Camera, Info } from 'lucide-react';
...
const DigitalCard = () => {
    ...
    const handleHistoryClick = (item: { meal: string; time: string; dayOffset: number; type: string }) => {
        // Calculate the correct day index based on offset
        const today = new Date().getDay(); // 0 (Sun) - 6 (Sat)
        // MENU_DATA starts from Sunday (Index 0)

        const targetDayIndex = (today + item.dayOffset + 7) % 7;
